<template>
  <main class="admin-contents-page">
    <aside class="admin-content-nav">
      ...
      <div class="filters">
        <button v-for="item in filters" :key="item.value" type="button" :class="{ active: activeType === item.value }" @click="activeType = item.value; loadContents()">{{ item.label }}</button>
      </div>
    ...
    <ContentEditor v-if="draft" v-model="draft" v-model:section="activeSection" @upload="uploadAsset" />
    ...
    <ContentPublishPanel v-if="draft" :content="draft" :validation="validation" :save-state-label="saveStateLabel"
      @preview="preview" @publish="publish" @unpublish="unpublish" @trash="trash" @focus-field="focusField" />
...
import {
  createAdminContent, fetchAdminContent, fetchAdminContents, publishAdminContent,
  trashAdminContent, unpublishAdminContent, updateAdminContent, uploadAdminContentAsset,
...
let saveTimer
let searchTimer
const filters = [{ label: '전체', value: '' }, { label: 'Show', value: 'show' }, { label: 'Project', value: 'project' }]
...
async function loadContents() {
  try { contents.value = await fetchAdminContents({ type: activeType.value, q: query.value }) }
...
async function createContent() {
  draft.value = await createAdminContent({ type: activeType.value || 'show', title: '', credits: [], assets: [] })
  contents.value.unshift(draft.value)
}
async function saveDraft() {
  try { draft.value = await updateAdminContent(draft.value.id, draft.value); saveState.value = 'saved'; await loadContents() }
  catch (error) { saveState.value = 'failed'; notice.value = error.message }
}
