...
완전한 응답을 검증한 뒤 보인 레코드는 `biennale_last_seen_at`을 갱신하고 `biennale_miss_count=0`으로 초기화한다. 보이지 않는 같은 회차의 활성 레코드는 성공한 재수집마다 누락 횟수를 하나 늘리며, 연속 2회 누락될 때만 비활성화한다. 신규/보인 저장, 누락 조정, `crawl_runs` 성공 기록, `crawl_completed_at`, 임대 해제는 하나의 D1 배치에서 원자적으로 처리한다. 이 배치의 첫 문장은 임대 소유권과 만료 시각을 다시 검증하고, 수집 중 임대가 만료·회수·초기화되었다면 SQL 오류로 전체 배치를 롤백한다. 성공 후 동일 회차는 재실행하지 않으며, 요청의 `force` 같은 쿼리나 본문 값으로 이 게이트를 우회할 수 없다.
...
광주미디어아트플랫폼(GMAP)   브라우저 렌더 → 제목/날짜범위        2건
광주시립미술관               단순 fetch + 기존 파서(브라우저 X)    32건
팔복예술공장(전북)           단순 fetch (.list-junbox-d2 + 일정:) 9건
```

보류(구조상 어려움):

```txt
우진문화공간(전북)   목록·상세에 전시기간 없음(등록일+포스터 이미지뿐) → 수동 권장
예울마루(전남/여수)  월별 캘린더 UI(탭 클릭 후 로드) → 추가 작업 필요
인스타(뽕뽕·빈틀)    로그인 벽
```
...
```txt
robots.txt가 /event.es를 차단하고 /menu.es만 허용하지만, /menu.es는 302로
/event.es(차단)로 리다이렉트한다. 즉 전시목록은 차단 경로에만 존재 → robots를
지키면서 수집할 방법이 없어 스크레이퍼에서 비활성화했다(코드는 보존).
재가동은 사이트 허락 또는 공식 API가 있을 때만.
```
